...
function qrcode($url,$path='data/upload/qrcode/qr.png',$size=4){
...
function all_childs($agency_id = 0){
    $pid = M('ucenter_member')->where('id='.$agency_id)->getField('pid');
    $all_ids = array();
    $yiji_ids = array();
    $erji_ids = array();
    $sanji_ids = array();
    $yiji = M('ucenter_member')->where('puid='.$agency_id)->field('id')->select();
    $yiji_ids = array_column($yiji,'id');
    if(!empty($yiji_ids)){
        foreach ($yiji_ids as $i1){
            $erji = M('ucenter_member')->where('puid='.$i1)->field('id')->select();
            if($erji){
                $erji_ids = array_merge($erji_ids,array_column($erji,'id'));
            }
        }
    }
    if(!empty($erji_ids) && $pid == 1){
        foreach ($erji_ids as $i2){
            $sanji = M('ucenter_member')->where('puid='.$i2)->field('id')->select();
            if($sanji){
                $sanji_ids = array_merge($sanji_ids,array_column($sanji,'id'));
            }
        }
    }
    $all_ids = array_merge($all_ids,$yiji_ids,$erji_ids,$sanji_ids);
    return $all_ids;
}

function all_one_childs($agency_id = 0){
    $all_ids = array();
    $yiji_ids = array();
    $erji_ids = array();
    $sanji_ids = array();
  ...
  /* if(!empty($yiji_ids)){
    ...
    $all_ids = array_merge($all_ids,$yiji_ids);
    return $all_ids;
}

function all_agency_childs($agency_id = 0){
    $yiji = M('ucenter_member')->where(array('puid'=>$agency_id,'is_agency'=>1))->field('id')->select();
    $yiji_ids = array_column($yiji,'id');
    /* if(!empty($yiji_ids)){
          foreach ($yiji_ids as $i1){
              $erji = M('ucenter_member')->where('puid='.$i1)->field('id')->select();
